...
import sys, traceback, codecs, os, stat
try:
    import dbus
    DBUS_AVAIL = True
except ImportError:
...
class PidginInfo:
    ...
    def writeOutput(self):
        ...
        if self.options.template == None:
            ...
            template = "${color0}${font Webdings:size=14})${font}${color}   ${color2}[alias] ([group]) - [status]${color}"
        else:
...
def main():
    ...
    if options.version == True:
        print >> sys.stdout,"conkyPidgin v.2.10"
    else:
